...
class PaginationServiceTest {
    ...
    @MethodSource
    @ParameterizedTest(name = "[{index}] 현재 페이지: {0}, 총 페이지: {1} => {2}") //여기에다 값을 여러 번 연속저긍로 주입해서 동일한 대상 메소드를 여러 번 테스트하면서 입출력 값을 볼 수 있는 기능
    //마지막 매개변수: 검증하고 싶은 값
    ...
    static Stream<Arguments> givenCurrentPageNumberAndTotalPages_whenCalculating_thenReturnsPaginationBarNumbers(){
        return Stream.of(
                Arguments.arguments(0, 13, List.of(0, 1, 2, 3, 4)), //123개의 게시글 10씩 묶어서 하면 12페이지 나머지깍지 해서 총 13페이지
    ...
    @Test
    //현재 페이지네이션 기능이 생각하고 있는 기본값(페이지네이션 바의 길이)가 5라는 점을 테스트 코드를 통해 드러내고 싶어서 테스트 코드 작성.
